...
Copyright 2012  Iuhas I. Daniel  (email : iuhas.daniel@igate.ro)
...
class Twitter_fetch extends WP_Widget {
	...
	function Twitter_Fetch() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'twitter', 'description' => __('Fetch your tweets from Twitter.com', 'twitter') );
	...
	function widget( $args, $instance ) {
		...
		$displayoptions = isset( $instance['display_options'] ) ? $instance['display_options'] : false;
		
		/* getting data */
		$error = false;
		require_once( ABSPATH . 'wp-includes/class-snoopy.php');
		$snoopy = new Snoopy;
		$response = @$snoopy->fetch("http://api.twitter.com/1/users/show/".$username.".json");
		if ($response) $user_data = json_decode($snoopy->results, true); else $error = true;
		$response = @$snoopy->fetch("http://api.twitter.com/1/statuses/user_timeline/".$username.".json");
		...
		if(!$error):
			...
			foreach($tweets as $tweet):
				...
				$twit_data['tweets'][$i]['retweet_count'] = $tweet['retweet_count'];
				$i++;
			endforeach;
		...
		if (!file_exists($cachefile) || (file_exists($cachefile) && (filemtime($cachefile) + $interval) < time())) :	
			if(!$error && is_array($twit_data['tweets'])):
				...
				foreach($twit_data['tweets'] as $tweet):
					...
					if ($displaydate) {
						$result .= '<a class="twitter-date" href="'.$link.'" rel="nofollow">'.$this->timeSince(abs(strtotime($tweettime . " GMT")),time()). '</a>';
					}
					
					if ($displayoptions) {
						$result .= '<div class="twitter-options">';
						$result .= '<div class="twitter-options-left">retweeted ' . $tweet['retweet_count'] . ' times</div>';
						$result .= '<div class="twitter-options-right">';
						$result .= '<a target="_blank" href="https://twitter.com/intent/tweet?in_reply_to=' . $tweet['id'] . '" title="Reply"><img src="'.$siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/images/reply.png" border="0" width="16" height="16" alt="Reply" /></a>';
						$result .= '<a target="_blank" href="https://twitter.com/intent/retweet?tweet_id=' . $tweet['id'] . '" title="Retweet"><img src="'.$siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/images/retweet.png" border="0" width="16" height="16" alt="Retweet" /></a>';
						$result .= '<a target="_blank" href="https://twitter.com/intent/favorite?tweet_id=' . $tweet['id'] . '" title="Favourite"><img src="'.$siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/images/favourite.png" border="0" width="16" height="16" alt="Favourite" /></a>';
						$result .= '</div>';
	...
	function form( $instance ) {
        ...
        <p>
			<input class="checkbox" type="checkbox" <?php if ($instance['display_options']) echo 'checked="checked" '; ?>id="<?php echo $this->get_field_id('display_options'); ?>" name="<?php echo $this->get_field_name('display_options'); ?>" />
				<label for="<?php echo $this->get_field_id('display_options'); ?>"><?php _e('Display options on tweets?', 'twitter'); ?></label>
